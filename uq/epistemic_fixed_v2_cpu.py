...
def main():
    ...
    if eval(test_pred_bool) is not False:
        ...
        test_pred(model=model,
                       ...
                       device=device,
                       **(vars(args)))
        logging.info("Test set predictions finished.")
    else:
        logging.info(f"Test set predictions assumed to be available as 'pred_{uq_method}.h5'.")

    if eval(detect_outliers_bool) is not False:
        logging.info("Detecting outliers on test sets.")
        ...
        logging.info("Outlier detection not called.")
