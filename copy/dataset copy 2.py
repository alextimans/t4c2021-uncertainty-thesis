...
class T4CDataset(Dataset):
    def __init__(self,
        ...
        if file_filter is None:
            assert dataset_type in ["train", "val", "test"], f"{dataset_type=} invalid."
            self.file_filter = "**/" + dataset_type + "/*8ch.h5"
        else:
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if (file_idx == len(self.files)): # Last file case idx 288
            file_idx -= 1
            start_hour = MAX_ONE_DAY_SMP_IDX # Replicate last full sample 22-24h
        elif (file_idx + 1 == len(self.files) and start_hour > MAX_ONE_DAY_SMP_IDX): # Idx 264-287
        ...
        assert (type(X) == torch.Tensor) and (type(y) == torch.Tensor)
